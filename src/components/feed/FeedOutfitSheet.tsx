...
export const FeedOutfitSheet = ({ item, open, onOpenChange }: FeedOutfitSheetProps) => {
  ...
  const handleSaveFullLook = async () => {
    ...
    try {
      const { error } = await supabase.from("dream_items").insert({
        ...
        image_url: item.image,
        item_type: "outfit",
        garments_json: JSON.parse(JSON.stringify(item.garments)),
      });
      if (error) throw error;
      setOutfitSaved(true);
  ...
  const handleShareOutfit = async () => {
    const shareData = {
      title: item.title,
      text: `Check out "${item.title}" by ${item.username} on VORA`,
  ...
  const handleAskAIOutfit = () => {
    onOpenChange(false);
    const garmentsSummary = item.garments.map(g => `${g.name} by ${g.brand} (${g.category})`).join("|");
    navigate(`/chat?outfit_name=${encodeURIComponent(item.title)}&outfit_garments=${encodeURIComponent(garmentsSummary)}`);
  };
